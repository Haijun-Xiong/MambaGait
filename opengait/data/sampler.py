...
class TripletSampler(tordata.sampler.Sampler):
    def __init__(self, dataset, batch_size, batch_shuffle=False):
        ...
        if len(self.batch_size) != 2:
            raise ValueError(f"batch_size should be (P x K) not {batch_size}")
        self.batch_shuffle = batch_shuffle

        self.world_size = dist.get_world_size()
        if (self.batch_size[0]*self.batch_size[1]) % self.world_size != 0:
            raise ValueError(
                f"World size ({self.world_size}) is not divisible by batch_size ({batch_size[0]} x {batch_size[1]})"
            )
    ...
    def __iter__(self):
        while True:
            ...
            yield sample_indices[self.rank:total_size:self.world_size]
...
class InferenceSampler(tordata.sampler.Sampler):
    def __init__(self, dataset, batch_size):
        ...
        if batch_size % world_size != 0:
            raise ValueError(
                f"World size ({world_size}) is not divisible by batch_size ({batch_size})"
            )
...
class CommonSampler(tordata.sampler.Sampler):
    def __init__(self,dataset,batch_size,batch_shuffle):
        ...
        if not isinstance(self.batch_size, int):
            raise ValueError(f"batch_size shoude be (B) not {batch_size}")
        self.batch_shuffle = batch_shuffle

        self.world_size = dist.get_world_size()
        if self.batch_size % self.world_size !=0:
            raise ValueError(
                f"World size ({self.world_size}) is not divisble by batch_size ({batch_size})"
            )
        self.rank = dist.get_rank() 
