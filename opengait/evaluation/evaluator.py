...
def single_view_gallery_evaluation(feature, label, seq_type, view, dataset, metric):
    probe_seq_dict = {'CASIA-B': {'NM': ['nm-05', 'nm-06'], 'BG': ['bg-01', 'bg-02'], 'CL': ['cl-01', 'cl-02'], 'Overall': ['nm-05', 'nm-06', 'bg-01', 'bg-02', 'cl-01', 'cl-02']},
                      'OUMVLP': {'NM': ['00']},
...
def evaluate_indoor_dataset(data, dataset, metric='euc', cross_view_gallery=False):
    ...
    # np.save('./ske_Gait3D_feature.npy', feature, allow_pickle=True)
    # np.save('./ske_Gait3D_label.npy', label, allow_pickle=True)
    # np.save('./ske_Gait3D_seq.npy', np.array(seq_type), allow_pickle=True)

    if dataset not in ('CASIA-B', 'OUMVLP', 'CASIA-E', 'SUSTech1K'):
        raise KeyError(f"DataSet {dataset} hasn't been supported !")
...
def GREW_submission(data, dataset, metric='euc'):
    ...
    with open(save_path, "w") as f:
        ...
        print(f"GREW result saved to {os.getcwd()}/{save_path}")
    return
...
def HID_submission(data, dataset, rerank=True, metric='euc'):
    ...
    with open(save_path, "w") as f:
        ...
        for i in range(len(idx)):
            f.write(f"{probe_y[i]},{gallery_y[idx[i, 0]]}\n")
        print(f"HID result saved to {os.getcwd()}/{save_path}")
    return
...
def evaluate_Gait3D(data, dataset, metric='euc'):
    ...
    results = {
        f'scalar/test_accuracy/Rank-{r}': cmc[r - 1] * 100 for r in [1, 5, 10]
    }
...
def evaluate_CCPG(data, dataset, metric='euc'):
    ...
    for i in range(1):
        ...
        msg_mgr.log_info(f'CL: {de_diag(acc[0, :, :, i], True)}')
        msg_mgr.log_info(f'UP: {de_diag(acc[1, :, :, i], True)}')
        msg_mgr.log_info(f'DN: {de_diag(acc[2, :, :, i], True)}')
        msg_mgr.log_info(f'BG: {de_diag(acc[3, :, :, i], True)}')
    return result_dict
