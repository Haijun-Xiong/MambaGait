...
def get_valid_args(obj, input_args, free_keys=[]):
    ...
    for k, v in input_args.items():
        if k in expected_keys:
            expected_args[k] = v
        elif k not in free_keys:
            unexpect_keys.append(k)
    if unexpect_keys != []:
        logging.info(
            f"Find Unexpected Args({', '.join(unexpect_keys)}) in the Configuration of - {obj.__name__} -"
        )
...
def is_list(x):
    return isinstance(x, (list, nn.ModuleList))


def is_dict(x):
    return isinstance(x, (dict, OrderedDict, Odict))
...
def MergeCfgsDict(src, dst):
    for k, v in src.items():
        if (k not in dst.keys()) or (type(v) != type({})):
            dst[k] = v
        elif is_dict(src[k]) and is_dict(dst[k]):
