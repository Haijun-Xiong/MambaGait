    ...
    def write_to_tensorboard(self, summary):
        ...
        for k, v in summary.items():
            ...
            if module_name not in self.writer_hparams:
                self.log_warning(
                    f'Not Expected --Summary-- type [{k}] appear!!!{self.writer_hparams}'
                )
                continue
            board_name = k.replace(f"{module_name}/", '')
            writer_module = getattr(self.writer, f'add_{module_name}')
...
def get_msg_mgr():
    return noop if torch.distributed.get_rank() > 0 else msg_mgr
