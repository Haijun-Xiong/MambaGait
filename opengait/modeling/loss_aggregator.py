...
class LossAggregator(nn.Module):
    ...
    def _build_loss_(self, loss_cfg):
        ...
        return get_ddp_module(Loss(**valid_loss_arg).cuda())
    ...
    def forward(self, training_feats):
        ...
        for k, v in training_feats.items():
            if k in self.losses:
                ...
                raise ValueError(
                    f"The key {k} in -Trainng-Feat- should be stated in your loss_cfg as log_prefix."
                )
            elif is_tensor(v):
                _ = v.mean()
                loss_info[f'scalar/{k}'] = _
                loss_sum += _
                get_msg_mgr().log_debug(f"Please check whether {k} needed in training.")
            else:
                raise ValueError(
