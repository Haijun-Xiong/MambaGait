...
class BaseModel(MetaModel, nn.Module):
    ...
    def get_scheduler(self, scheduler_cfg):
        ...
        return Scheduler(self.optimizer, **valid_arg)
    ...
    def _load_ckpt(self, save_name):
        ...
        if self.training:
            if not self.engine_cfg["optimizer_reset"] and 'optimizer' in checkpoint:
                ...
                self.msg_mgr.log_warning(f"Restore NO Optimizer from {save_name} !!!")
            if not self.engine_cfg["scheduler_reset"] and 'scheduler' in checkpoint:
                self.scheduler.load_state_dict(
                    checkpoint['scheduler'])
            else:
                self.msg_mgr.log_warning(f"Restore NO Scheduler from {save_name} !!!")
        self.msg_mgr.log_info(f"Restore Parameters from {save_name} !!!")
    ...
    def inputs_pretreament(self, inputs):
        ...
        if len(seqs_batch) != len(seq_trfs):
            raise ValueError(
                f"The number of types of input data and transform should be same. But got {len(seqs_batch)} and {len(seq_trfs)}"
            )
        ...
        if self.engine_cfg['enable_float16']:
            ...
            if scale != self.Scaler.get_scale():
                self.msg_mgr.log_debug(
                    f"Training step skip. Expected the former scale equals to the present, got {scale} and {self.Scaler.get_scale()}"
                )
                return False
        else:
            loss_sum.backward()
    ...
    def inference(self, rank):
        ...
        pbar = tqdm(total=total_size, desc='Transforming') if rank == 0 else NoOp()
        batch_size = self.test_loader.batch_sampler.batch_size
        rest_size = total_size
        info_dict = Odict()
        for inputs in self.test_loader:
            ipts = self.inputs_pretreament(inputs)
            with autocast('cuda', enabled=self.engine_cfg['enable_float16']):
    ...
    def run_test(model):
        ...
        if torch.distributed.get_world_size() != evaluator_cfg['sampler']['batch_size']:
            raise ValueError(
                f"The batch size ({evaluator_cfg['sampler']['batch_size']}) must be equal to the number of GPUs ({torch.distributed.get_world_size()}) in testing mode!"
            )
